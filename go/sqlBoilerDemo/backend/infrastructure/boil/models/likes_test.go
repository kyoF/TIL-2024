...
func testLikeToOneTweetUsingTweet(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if check.TweetID != foreign.TweetID {
		t.Errorf("want: %v, got %v", foreign.TweetID, check.TweetID)
	}

	ranAfterSelectHook := false
	AddTweetHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Tweet) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := LikeSlice{&local}
	if err = local.L.LoadTweet(ctx, tx, false, (*[]*Like)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testLikeToOneSetOpTweetUsingTweet(t *testing.T) {
	var err error
	...
	for i, x := range []*Tweet{&b, &c} {
		...
		if a.TweetID != x.TweetID {
			t.Error("foreign key was wrong value", a.TweetID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.TweetID))
		reflect.Indirect(reflect.ValueOf(&a.TweetID)).Set(zero)
		...
		if a.TweetID != x.TweetID {
			t.Error("foreign key was wrong value", a.TweetID, x.TweetID)
		}
	}
}
...
var (
	likeDBTypes = map[string]string{`ID`: `int`, `UserID`: `varchar`, `TweetID`: `char`, `CreatedAt`: `timestamp`}
	_           = bytes.MinRead
)
