...
func testUserToManyTweets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadTweets(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpTweets(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `UserID`: `varchar`, `Name`: `varchar`, `Email`: `varchar`, `Profile`: `text`, `Password`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_           = bytes.MinRead
)
