...
func testTweetToManyLikes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TweetSlice{&a}
	if err = a.L.LoadLikes(ctx, tx, false, (*[]*Tweet)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTweetToManyAddOpLikes(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.TweetID != first.TweetID {
			t.Error("foreign key was wrong value", a.TweetID, first.TweetID)
		}
		if a.TweetID != second.TweetID {
			t.Error("foreign key was wrong value", a.TweetID, second.TweetID)
		}
...
func testTweetToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Tweet)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testTweetToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	tweetDBTypes = map[string]string{`ID`: `int`, `TweetID`: `char`, `UserID`: `varchar`, `Text`: `text`, `LikesCount`: `int`, `CreatedAt`: `timestamp`}
	_            = bytes.MinRead
)
...
func testTweetsUpsert(t *testing.T) {
	...
	if len(mySQLTweetUniqueColumns) == 0 {
